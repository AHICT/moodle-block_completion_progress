...
//TODO require_once($CFG->dirroot . '/notes/lib.php');
...
$PAGE->navbar->add(
    $strtitle,
    new moodle_url('/blocks/completion_progress/overview.php', ['instanceid' => $instanceid, 'courseid' => $courseid])
);
...
$filter = new overview_filter($context, $instanceid, $table->uniqueid);
...
//TODO default to the first student-archetype role in use: $filterset->add_filter(new integer_filter('roles', filter::JOINTYPE_DEFAULT, [5]));
$table->set_filterset($filterset);

if ($table->is_downloading($download, 'completion_progress-' . $course->shortname)) {
    //TODO fix the filterset not being applied.
...
// For now we must compute progress percentages because the user might sort by that column and the
// dynamic loader will need the values to be there. A better idea may come later.
